...
void ImprimeCabeceraCliente(){
    cout<<left<<setw(40)<<"Cliente"<<setw(30)<<"CODIGO DE CUENTA"
            <<setw(14)<<"MONEDA"<<"SALDO INICIAL"<<endl;
}
...
void LeerCliente(char &moneda){
    ...
    cin>>moneda>>saldo;
    if (moneda == '$') cout<<setw(26)<<"Dolar"<<setw(12)<<"$  ";
    else if (moneda == 'S') cout<<setw(26)<<"Soles"<<setw(12)<<"S  ";
    else cout<<right<<setw(26)<<"Euros"<<setw(12)<<"&  ";
    cout<<saldo<<endl;
    ImprimeLinea('=', MAX_CAR_LIN);
}
...
double ActualizarMontoAMonedaCuenta(char moneda, char moneda_cuenta,
                                    ...
                                    double euro_cambio){
    if (moneda_cuenta == 'S'){
        if (moneda=='$')    return monto * dolar_cambio;
        else    return monto * euro_cambio;
        
    }
    else if (moneda_cuenta == '$'){
        if (moneda == 'S')  return monto / dolar_cambio;
        else    return monto * euro_cambio / dolar_cambio;
    }
    else{
        if (moneda == 'S')  return monto / euro_cambio;
        // Quiero pasar todo a euro de dolar
        // dolar * dolar_cambio / euro_cambio
        else return monto * dolar_cambio / euro_cambio;
    }
}
