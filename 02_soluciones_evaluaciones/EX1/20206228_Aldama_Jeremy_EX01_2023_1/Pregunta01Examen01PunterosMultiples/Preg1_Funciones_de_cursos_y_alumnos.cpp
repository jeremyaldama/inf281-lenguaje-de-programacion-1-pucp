...
void cargarAlumnos(int*&alumnos_cod, int**&alumnos,
    ...
    while (1){
        ...
        if (num == cap){
            IncrementarAlumnos(alumnos_cod, alumnos, alumnos_nom_mod,
                    num, cap);
        }
        
        AgregarAlumnos(alumnos_cod, alumnos, alumnos_nom_mod, cod, escala,
                porcentaje, nombre, mod, num-1);
        num++;
    }
}

void AgregarAlumnos(int*alumnos_cod, int**alumnos, char***alumnos_nom_mod, 
        int cod, int escala, int porcentaje, char*nombre, char mod, int i){
    alumnos_cod[i] = cod;
...
void IncrementarAlumnos(int*&alumnos_cod, int**&alumnos, char***&alumnos_nom_mod,
                    int&num, int&cap){
    cap += INCREMENTO;
