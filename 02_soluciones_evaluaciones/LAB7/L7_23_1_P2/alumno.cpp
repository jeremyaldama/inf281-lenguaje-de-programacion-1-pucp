...
const class nota* alumno::GetLnotas() const {
    return lnotas;
}
...
void alumno::operator+=(class nota&grade) {
    int i;
    for (i = 0; lnotas[i].GetCiclo(); i++);
//    lnotas[i] = grade;
    lnotas[i].copiar(grade);
    
//    lnotas[i].SetCodigo(grade.GetCodigo());
//    lnotas[i].SetCiclo(grade.GetCiclo());
//    lnotas[i].SetNota(grade.GetNota());
//    char c[100];
//    grade.GetCodcurso(c);
//    lnotas[i].SetCodcurso(c);
}
...
ofstream& operator<<(ofstream&arch, alumno&al) {
    ...
    for (int i = 0; notas[i].GetCiclo(); i++) {
        notas[i].GetCodcurso(no);
        arch << no << " " << notas[i].GetCiclo() << "  ";
        arch << notas[i].GetNota() << endl;
    }
}